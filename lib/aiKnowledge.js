...
export function buildKnowledge() {
  ...
  const pricing = dictionaries.en.services.pricing;
  lines.push(`\nWEBSITE PACKAGES (STARTING prices in IDR — final quote depends on scope; discovery chat is free):`);
  pricing.items.forEach((p) => {
    lines.push(`- ${p.name}: from ${p.price} — ${p.desc} Includes: ${p.features.join("; ")}.`);
  });
  lines.push(`- ${pricing.enterprise.name}: custom quote — ${pricing.enterprise.desc}`);
  lines.push(`Note: ${pricing.footnote}`);
