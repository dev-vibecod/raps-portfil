  ...
  return (
    <Reveal>
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="surface surface-hover group block overflow-hidden rounded-3xl"
      >
        <div className="grid md:grid-cols-2">
          ...
          <div className="relative aspect-[16/10] overflow-hidden border-b border-line md:border-b-0 md:border-r">
            ...
            />
            <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2.5 py-1 text-2xs font-medium text-emerald-300 ring-1 ring-emerald-500/30">
          ...
          <div className="flex flex-col justify-center p-7 sm:p-9">
            ...
            <div className="mt-5 flex flex-wrap gap-6">
              {project.metrics.map((m) => (
                <div key={m.label}>
                  <p className="meta text-xl font-semibold text-white">{m.value}</p>
                  <p className="text-2xs font-mono uppercase tracking-label text-mist/65">{m.label}</p>
                </div>
              ))}
            </div>
