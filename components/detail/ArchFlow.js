      ...
      <div className="mt-6 flex flex-col flex-wrap items-stretch gap-2 sm:flex-row sm:items-center">
        {architecture.flow.map((node, i) => (
          <div key={node} className="flex items-center gap-2">
            <div className="rounded-xl border border-iris-500/25 bg-iris-500/10 px-3.5 py-2.5 text-center text-sm font-medium text-white">
              {node}
            </div>
      ...
      {architecture.layers && (
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {architecture.layers.map((layer) => (
            <div key={layer.name} className="rounded-2xl border border-line bg-veil p-4">
              <p className="font-mono text-2xs font-semibold uppercase tracking-label text-iris-400">{layer.name}</p>
              <ul className="mt-2 space-y-1.5">
                {layer.items.map((it) => (
                  <li key={it} className="flex gap-2 text-sm text-mist/70">
